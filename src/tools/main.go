...
func initRepair(db *pg.DB) error {
	...
	for i < 2000 {
		...
		count++
		if count == 5000 {
			if _, err := db.CopyFrom(&buf,
				`COPY asset_repair(epc_code, create_time) FROM STDIN WITH CSV`); err != nil {
				return err
			}
...
func initChanges(db *pg.DB) error {
	...
	for i < 20000 {
		t1 := int(r.Int31n(50))
		t2 := t.AddDate(0, 0, t1)
		buf.WriteString(fmt.Sprintf("Change_%d,%s\n",
			i, t2.Format("2006-01-02 15:04:05")))
		count++
		if count == 10000 {
			if _, err := db.CopyFrom(&buf,
				`COPY asset_change(epc_code, create_time) FROM STDIN WITH CSV`); err != nil {
				return err
			}
