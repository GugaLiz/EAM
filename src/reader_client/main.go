	...
	for {
		...
		case <-ticker.C:
			...
			if can {
				...
				if msg, err = doJob(setting); err != nil {
					msg = err.Error()
				...
				web.UpdateUploadStatus(lastUpload, msg, ok)
				web.UpdateCommonStatus(setting)
			}
			...
			if doHeartBeat {
				if devId, err := reader.GetRCUId(); err != nil {
					...
					if err := reader.SendHeartBeat(setting.Host, setting.Port, devId); err != nil {
...
func createDataPath() {
	...
	if ok, err, _ := utils.PathExists(LogDataPath); !ok {
		if err = os.MkdirAll(LogDataPath, os.ModePerm); err != nil {
			log.Fatal(err)
		}
	}
}
...
func doJob(setting *reader.Setting) (string, error) {
	var (
		devId      string
		uploadJson []byte
		tags       []reader.Tag
	...
	if tags, err = reader.GetEPCList(1); err != nil {
		log.Println(err)
		return "", err
	}
	tagsCount := len(tags)
	log.Printf("inventory count:%d\n", tagsCount)
	dictTags := make(map[string]reader.Tag)
	for i := 0; i < tagsCount; i++ {
		tag := &tags[i]
		tag.LastCheck = &utcNow
		tag.FirstFound = &utcNow
		dictTags[tag.EPC] = *tag
	}

	dataFile := filepath.Join(DataPath, "data.json")
	...
	changes := make([]reader.Tag, 0)
	losts := make([]reader.Tag, 0)
	adds := make([]reader.Tag, 0)
	...
	logFile := filepath.Join(LogDataPath, logName)
	logNotSend := filepath.Join(LogDataPath, fmt.Sprintf("%s_notsend.json", now1))
	...
	for loop {
		...
		log.Printf("file:%s\n", logName)
		if err = reader.SendToRemote(setting.Host, setting.Port, devId,
			haveUpdate, logName, uploadJson); err != nil {
