...
func GetEPCList(sleep int) ([]Tag, error) {
	var (
		err      error
		comName  string
		readerId string
		...
		if resp, err := WriteCmdSerial3(cmdGetInventoryBuffer, ser); err != nil {
			...
			if sz == 1 {
				...
				if tagCount > 1 {
					...
					for leftTagCount > 0 {
						lbuf := make([]byte, sz+5)
						if _, err = io.ReadFull(ser, lbuf); err != nil {
							log.Println("read full error:")
							log.Println(err)
						} else {
							buf2 := lbuf[4 : len(lbuf)-1] // remove head and check byte
							sz2 := len(buf2)
							if sz2 >= 6 {
...
func getTagInfo(buf []byte, readerId string) Tag {
	sz := len(buf)
...
func WriteSerial(data []byte, rwc io.ReadWriteCloser) ([]byte, error) {
	...
	header := make([]byte, 1)
	if n, err = rwc.Read(header); err != nil {
		return nil, err
	}
	if n == 0 {
	...
	szbuf := make([]byte, 1)
	if n, err = rwc.Read(szbuf); err != nil {
		return nil, err
	}
	sz := int(szbuf[0])
	buf := make([]byte, sz)
